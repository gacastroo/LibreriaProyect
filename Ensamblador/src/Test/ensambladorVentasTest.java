...
public class ensambladorVentasTest {
    ...
    public void testBuscarVentaPorCliente() {
        ...
        String resultado = EnsambladorVentas.BuscarVentaPorCliente(ventas, clientes);
        assertEquals("\nVentas para el cliente Juan:\nVenta 1\n", resultado);

        resultado = EnsambladorVentas.BuscarVentaPorCliente(ventas, Arrays.asList(cliente2));
        assertEquals("No se encontraron ventas para el cliente María", resultado);

        resultado = EnsambladorVentas.BuscarVentaPorCliente(ventas, Arrays.asList(new Cliente("Pedro","asd", "pedro@example.com", "555", LocalDate.now())));
        assertEquals("El cliente Pedro no está registrado", resultado);
    }
    ...
    public void testBuscarVentasPorLibro() {
        ...
        List<Archivos> archivos = new ArrayList<>();
        List<Ventas> ventas = new ArrayList<>();
        ventas.add(new Ventas(LocalDate.now(), Arrays.asList(libro1), new Cliente("Juan", "juan@example.com", "123456789", "Calle Principal", LocalDate.now())));

        // Llamada al método y verificación
        Ventas resultado = EnsambladorVentas.BuscarVentasPorLibro(libros, ventas);
        assertEquals(ventas.get(0), resultado);

        resultado = EnsambladorVentas.BuscarVentasPorLibro(Arrays.asList(new Libros("Libro3", "Autor3", "Genero3", 30.0)), ventas);
    ...
    public void testTotalVentas() {
        ...
        // Llamada al método y verificación
        String resultado = EnsambladorVentas.TotalVentas(ventas);
        assertEquals("El total de las ventas es: 8.0", resultado); // Hasta ahora, solo hemos creado ventas, no se ha calculado el total.
    }
}
